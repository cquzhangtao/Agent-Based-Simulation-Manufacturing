...
public class CycleTimeChart extends JFrame {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7565661890673533545L;
	...
	public CycleTimeChart(ReleaseDataset releaseDataset) {
		...
		chart = ChartFactory.createXYLineChart(releaseDataset.get(0).product,
				"Time(day)", "Cycle Time (Hour)", getDataset(0),
		...
		multiXYchart = ChartFactory.createXYLineChart(
				releaseDataset.get(0).product, "Time(day)", "Cycle Time (Hour)",
		...
		barchart = ChartFactory.createBarChart("Average CycleTime", "Product",
				"Cycle Time (Hour)",
...
//					.size(); i++) {
//				series.add(i, releaseDataset.allReleaseDataset.cycleTimebyDay.get(i));
//				series.add(i + 1,
//						releaseDataset.allReleaseDataset.cycleTimebyDay.get(i));
//			}
//			dataset.addSeries(series);
//			return dataset;
		}

		for (int i = 0; i < releaseDataset.get(index).cycleTimebyDay.size(); i++) {
			series.add(i, releaseDataset.get(index).cycleTimebyDay.get(i));
			series.add(i + 1, releaseDataset.get(index).cycleTimebyDay.get(i));
		}
...
//				}
				...
				for (int i = 0; i < releaseDataset.get(index[j]-1).cycleTimebyDay
						.size(); i++) {
					series.add(i, releaseDataset.get(index[j]-1).cycleTimebyDay.get(i));
					series.add(i + 1,
							releaseDataset.get(index[j]-1).cycleTimebyDay.get(i));
				}
				dataset.addSeries(series);
			}
		...
		if (selectedBuffer == null || selectedBuffer.length < 2) {
			...
			for (int i = 0; i < selectedBuffer.length; i++) {
				if (selectedBuffer[i] == 0) {
					if (type == 0 || type == 1)
						result.addValue(data.allReleaseDataset.avgCycleTime, series1,
								"All");
					if (type == 0 || type == 2)
						result.addValue(data.allReleaseDataset.maxCycleTime, series2,
								"All");
					if (type == 0 || type == 3)
						result.addValue(data.allReleaseDataset.minCycleTime, series3,
								"All");
				} else {
					if (type == 0 || type == 1)
						result.addValue(data.get(selectedBuffer[i] - 1).avgCycleTime,
								series1,
								data.get(selectedBuffer[i] - 1).product);
					if (type == 0 || type == 2)
						result.addValue(data.get(selectedBuffer[i] - 1).maxCycleTime,
								series2,
								data.get(selectedBuffer[i] - 1).product);
					if (type == 0 || type == 3)
						result.addValue(data.get(selectedBuffer[i] - 1).minCycleTime,
								series3,
								data.get(selectedBuffer[i] - 1).product);
				}
			}
	...
	class CustomListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (rButton == average) {
				typeIndex = 1;
				barchart.setTitle("Average Cycle Time");
			} else if (rButton == max) {
				typeIndex = 2;
				barchart.setTitle("Max Cycle Time");
			} else if (rButton == min) {
				typeIndex = 3;
				barchart.setTitle("Min Cycle Time");
